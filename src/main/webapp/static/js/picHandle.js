 ...
 * 缩略Dom循环展示
 * @author yingjq
 * @param location dom存储位置，是个dom对象
 * @param domArr dom数组
 * 
 */
var domNumLocal = null;
var domArrLocal = [];
function domCircleDisplay(domArr, location){
	domArrLocal = domArr;
	domNumLocal = domArr.length;
	var domLis = "";
	if(domNumLocal <= 10){
		domLis += "<ul id='smallDoms'>";
		for(var i=0;i<domNumLocal;i++){
			domLis += "<li  style='list-style-type:none;float:left;height:170px;width:auto;margin-right:8px;'>"+domArr[i]+"</li>";
		}
		domLis += "</ul>";
	}else{
		domLis += "<div style='position:absolute;z-index:999;margin-top:12px;cursor:pointer' onclick='goDomLeft();'><img src='/ZSCY/static/image/arrowhead_left.png'  width='60px' height='60px' style='opacity:0.5;'/></div>"+
			"<ul style='list-style-type:none;margin-left:7px' id='smallDoms'>";
		for(var i=0;i<domNumLocal;i++){
			domLis += "<li  style='list-style-type:none;float:left;height:170px;width:auto;border:1px solid green;margin-right:8px'>"+domArr[i]+"</li>";
		}
		domLis += "</ul><div style='position:absolute;margin-left:390px;z-index:999;cursor:pointer;' onclick='goDomRight();'><img src='/ZSCY/static/image/arrowhead_right.png'  width='160px' height='160px' style='opacity:0.5;'/></div>";
	}
	var optionHtml = "<div class='row' style='heigth:180px;width:100%;padding:10px 5px 3px 5px'>"+domLis+"</div>";
	$(location).append(optionHtml);
}
function goDomLeft(){
	$("#smallDoms li").eq(0).before($("#smallDoms li").eq(domNumLocal-1));
}
function goDomRight(){
	if($("#smallDoms li").eq(0).next()){
		$("#smallDoms li").eq(domNumLocal-1).after($("#smallDoms li").eq(0)); 
	}
}